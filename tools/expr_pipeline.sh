...
TRAIN_SRC="$DATA_DIR/train.ko-en.tok.clean.bpe32k.$SRC"
TRAIN_TGT="$DATA_DIR/train.ko-en.tok.clean.bpe32k.$TGT"
VALID_SRC="$DATA_DIR/tst2016.en-ko.tok.bpe32k.$SRC"
VALID_TGT="$DATA_DIR/tst2016.en-ko.tok.bpe32k.$TGT"
TEST_SRC="$DATA_DIR/tst2017.en-ko.tok.bpe32k.$SRC"
TEST_TGT="$DATA_DIR/tst2017.en-ko.tok.bpe32k.$TGT"
...
-config $ONMT/config/config-transformer-base-2GPU.yml \
...
for model in `ls -tr $OUTPUT_DIR/models/*.pt`; do
    ...
    $ONMT/tools/multi-bleu-detok.perl $TEST_TGT < $OUTPUT_DIR/test/test.out >> $OUTPUT_DIR/test/test.tc.bleu
    $ONMT/tools/multi-bleu-detok.perl -lc $TEST_TGT < $OUTPUT_DIR/test/test.out >> $OUTPUT_DIR/test/test.lc.bleu

    echo "Step 4b: Evaluate Dev"
    echo $model >> $OUTPUT_DIR/test/valid.tc.bleu
    echo $model >> $OUTPUT_DIR/test/valid.lc.bleu
    $ONMT/tools/multi-bleu-detok.perl $VALID_TGT < $OUTPUT_DIR/test/valid.out >> $OUTPUT_DIR/test/valid.tc.bleu
    $ONMT/tools/multi-bleu-detok.perl -lc $VALID_TGT < $OUTPUT_DIR/test/valid.out >> $OUTPUT_DIR/test/valid.lc.bleu
done
