...
TRAIN_SRC="$DATA_DIR/train.ko-en.tok.clean.wb.bpe32k.$SRC"
TRAIN_TGT="$DATA_DIR/train.ko-en.tok.clean.wb.bpe32k.$TGT"
VALID_SRC="$DATA_DIR/tst2016.en-ko.tok.wb.bpe32k.$SRC"
VALID_TGT="$DATA_DIR/tst2016.en-ko.tok.$TGT"
TEST_SRC="$DATA_DIR/tst2017.en-ko.tok.wb.bpe32k.$SRC"
...
for model in `ls -tr $OUTPUT_DIR/models/*.pt`; do
    ...
    cat $OUTPUT_DIR/test/valid.out.bpe | sed -E 's/_//g' | sed -E 's/(@@ )|(@@ ?$)//g' > $OUTPUT_DIR/test/valid.out
    cat $OUTPUT_DIR/test/test.out.bpe | sed -E 's/_//g' | sed -E 's/(@@ )|(@@ ?$)//g' > $OUTPUT_DIR/test/test.out
