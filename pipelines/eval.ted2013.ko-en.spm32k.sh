...
TRAIN_SRC="$DATA_DIR/train.ko-en.tok.clean.spm32k.$SRC"
TRAIN_TGT="$DATA_DIR/train.ko-en.tok.clean.spm32k.$TGT"
VALID_SRC="$DATA_DIR/tst2016.en-ko.tok.spm32k.$SRC"
VALID_TGT="$DATA_DIR/tst2016.en-ko.tok.$TGT"
TEST_SRC="$DATA_DIR/tst2017.en-ko.tok.spm32k.$SRC"
...
for model in `ls -tr $OUTPUT_DIR/models/*.pt`; do
    ...
    echo "SPM decoding/detokenising target to match with references"
    mv $OUTPUT_DIR/test/test.out{,.spm}
    mv $OUTPUT_DIR/test/valid.out{,.spm}
    cat $OUTPUT_DIR/test/valid.out.spm | sed -E 's/ //g' | sed -E 's/▁/ /g' | sed -E 's/^ //g' > $OUTPUT_DIR/test/valid.out
    cat $OUTPUT_DIR/test/test.out.spm | sed -E 's/ //g' | sed -E 's/▁/ /g' | sed -E 's/^ //g' > $OUTPUT_DIR/test/test.out
